...
const createOperations: INodeProperties[] = [
...

	{
		...
		typeOptions: {
			loadOptions: {
				routing: {
					// ATTENZIONE: endpoint SINGOLARE
					request: { method: 'GET', url: '/v1/model/info' },
					output: {
						postReceive: [
							// la risposta che hai incollato è { data: [...] }
							{ type: 'rootProperty', properties: { property: 'data' } },
							{
								type: 'filter',
								properties: {
									// tieni solo i modelli immagine
							...
							},
							{
								type: 'setKeyValue',
								properties: {
									// etichetta leggibile nel dropdown
									name: '={{ $responseItem.model_name }}',
									// value inviato all'API (Regolo si aspetta il nome modello)
